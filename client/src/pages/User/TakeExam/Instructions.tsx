import React from 'react';
import { useNavigate } from 'react-router-dom';

interface InstructionsProps {
  examData: any;
  setView: any;
  startTimer: () => void;
}

const Instructions = ({ examData, setView, startTimer }: InstructionsProps) => {
  const navigate = useNavigate();
  return (
    <div className="flex flex-col items-center gap-5">
      <ul className="flex flex-col gap-1 text-white">
        <h1 className="text-2xl underline text-white">Instructions</h1>
        <li>Exam must be completed in {examData?.duration} seconds</li>
        <li>
          Exam will be submitted automatically after {examData?.duration}{' '}
          seconds.
        </li>
        <li>Once submitted, you can not change your answers.</li>
        <li>Do not refresh the page.</li>
        <li>
          You can use the <span className="font-bold">"Previous"</span> and{' '}
          <span className="font-bold">"Next"</span> buttons to navigate between
          questions.
        </li>
        <li>
          Total marks of the exam is{' '}
          <span className="font-bold">{examData.totalMarks}</span>.
        </li>
        <li>
          Passing marks of the exam is{' '}
          <span className="font-bold">{examData.passingMarks}</span>.
        </li>
      </ul>
      <div className="flex gap-2">
        <button
          className="rounded-lg bg-red-700 hover:bg-red-900 text-white h-8 font-semibold px-2 flex items-center"
          onClick={() => navigate('/')}
        >
          CLOSE
        </button>
        <button
          className="rounded-lg bg-green-700 hover:bg-green-900 text-white h-8 font-semibold px-2 flex items-center"
          onClick={() => {
            startTimer();
            setView({ section: 'questions' });
          }}
        >
          Start Exam
        </button>
      </div>
    </div>
  );
};

export default Instructions;
